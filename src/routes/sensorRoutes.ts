...
router.get("", authenticateUser([UserType.Admin, UserType.Operator, UserType.Viewer]), async (req, res, next) => {
  try {
    res.status(200).json(await getAllSensor(req.params.networkCode, req.params.gatewayMac));
  } catch (error) {
    next(error);
  }
});
...
router.delete("/:sensorMac", authenticateUser([UserType.Admin, UserType.Operator]), async (req, res, next) => {
  try {
    await deleteSensorByMac(req.params.networkCode, req.params.gatewayMac, req.params.sensorMac);
    res.status(204).send();
  } catch (error) {
    next(error);
  }
